...
3. Different path projections with risky growth rates

```{r}
# Define starting values
finres.uncertain <- data.frame(NULL, NULL, NULL)
...
finres.uncertain <- rbind(finres.uncertain, data.frame(t, sales, fixcosts, simulation = rep(r, 11)))
t <- 0 
}
...
# Plot series
ggplot() +
  geom_line(data = finres.uncertain, aes(x = t, y = nresult, group = simulation), color = "skyblue") +
